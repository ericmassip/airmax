...
from airmax.views.map.map_service import MapCity, MapData, get_map_data
...
@pytest.fixture
def frozen_clock():
    """`get_map_data` reads the clock itself. Patching it beats passing an instant in, which would put an argument
...
def test_two_stations_in_the_same_city_and_hour_collapse_into_one_bucket(frozen_clock):
    """The whole point of the map: Aalst is one shape on the map however many stations sit inside it. The bucket
...
def test_the_newest_bucket_start_is_the_hour_of_the_newest_reading_or_nothing():
    assert [a_map_data(hours).newest_bucket_start for hours in [[], [LATEST_HOUR - 1]]] == [
        None,
...
def test_the_opening_window_covers_the_newest_hours_and_nothing_older():
    ...
    assert [a_map_data(hours).latest_window_is_empty for hours in newest_hours] == [
        True,
        True,
        False,
    ]
